...
export class StorageService {
  ...
  private _modulesCache: FactorioModule[];

  get modulesCache(): FactorioModule[] {
  ...
  private _modulesChanged: Subject<FactorioModule[]> = new Subject();

  get craftingMachinesChanged(): Subject<FactorioCraftingMachine[]> {
    return this._craftingMachinesChanged;
  }

  get modulesChanged(): Subject<FactorioModule[]> {
    return this._modulesChanged;
  }
  ...
  public static clearLocalStorage() {
    localStorage.removeItem(StorageService.recipeStorageKey);
    localStorage.removeItem(StorageService.craftingMachinesStorageKey);
    localStorage.removeItem(StorageService.modulesStorageKey);
  }
  ...
  private static parseModules(json: any): FactorioModule[] {
    let result = [];

    for (let item of Object.values(json)) {
      if (Object.keys(item).includes("module_effects")) {
        let module = FactorioModule.Parse(item);
        result.push(module);
      }
    }
  ...
  private importData(file: File) {
    ...
    fileReader.onload = (data: any) => {
      ...
      let modules = StorageService.parseModules(json.item);

      this.storeRecepies(recepies);
      this.storeCraftingMachines(craftingMachines);
      this.storeModules(modules);
    };
  ...
  private storeModules(data: FactorioModule[]) {
